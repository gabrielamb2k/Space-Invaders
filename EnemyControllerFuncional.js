...
const updateDirectionAndVelocity = (controller) => {
  for (const row of controller.enemyRows) {
    if (controller.currentDirection === MovingDirection.right) {
      ...
      if (rightMost.x + rightMost.width >= controller.canvas.width) {
        return {
          ...
          xVelocity: 0,
          yVelocity: controller.defaultYVelocity,
        };
      }
      return {
        ...controller,
        xVelocity: controller.defaultXVelocity,
        yVelocity: 0,
      };
    }
    ...
    if (
      controller.currentDirection === MovingDirection.downLeft ||
      controller.currentDirection === MovingDirection.downRight
    ) {
...
const checkCollision = (controller) => {
  const newRows = controller.enemyRows
    .map((row) =>
      row.filter((enemy) => {
